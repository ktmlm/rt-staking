...
use crate::pos::{
    AccMgmt, Amount, BalanceDB, Power, Punishment, Score, Staking, Validator, ValidatorID,
    ValidatorIDRef,
};
...
impl Auth {
    pub fn new(
        ...
        let s =
            Staking::new(score_max, score_min_for_offline, validator_cap, cache_size).c(d!())?;
    ...
    pub fn stake_to<B: BalanceDB>(
        ...
        self.staking
            .stake_to(sender, validator, amount, true)
            .c(d!())
    }
    ...
    pub fn unstake_from<B: BalanceDB>(
        ...
        self.staking.unstake_from(sender, validator, amount).c(d!())
    }
    ...
    pub fn unstake_all<B: BalanceDB>(&mut self, sender: &mut AccMgmt<B>) -> Result<Amount> {
        self.staking.unstake_all(sender).c(d!())
    }
